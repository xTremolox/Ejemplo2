...
import com.protectos.ejemplo2.databinding.ActivityJuegoSuma510Binding
import com.protectos.ejemplo2.databinding.ScoresumaBinding
import kotlin.random.Random

class Actividad_Juego_Suma5_10 : AppCompatActivity() {
    private lateinit var vinculo: ActivityJuegoSuma510Binding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        vinculo= ActivityJuegoSuma510Binding.inflate(layoutInflater)
    ...
    private fun generateQuestion() {
        val num1 = Random.nextInt(600000,999999)
        val num2 = Random.nextInt(600000, 999999)
        vinculo.btnNum1.text = "$num1"
        vinculo.btnNum2.text = "$num2"
        suma=num1+num2

        val opciones = listOf(suma, suma + 100, suma -200).shuffled()
    ...
    private fun validar(rpta: Int) {
        if (rpta==suma) {
            ...
            vinculo.score.text = "$correctAnswers/30"
            ...
            if (correctAnswers == 30) {
                Toast.makeText(this, "¡Felicidades! Has acertado 30 veces.", Toast.LENGTH_LONG).show()
                Handler(Looper.getMainLooper()).postDelayed({
                    generateQuestion()
                }, 1000)
